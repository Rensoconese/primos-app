...
import { createPublicClient, http, type PublicClient, type WalletClient, type Account } from 'viem';
...
const RewardsPanel: React.FC<RewardsPanelProps> = ({ 
  ...
  // Inicializar publicClient si no se proporciona externamente
  useEffect(() => {
    if (externalPublicClient) {
      setPublicClient(externalPublicClient);
    } else if (userAddress) {
      // Crear un publicClient por defecto si no se proporciona uno
  ...
  }, [externalPublicClient, userAddress]);
  ...
  useEffect(() => {
    ...
    const fetchTokenBalance = async () => {
      try {
        // Leer el balance del token usando viem
        const balance = await publicClient.readContract({
          address: FIRE_DUST_ADDRESS as `0x${string}`,
          abi: FIRE_DUST_ABI,
          functionName: 'balanceOf',
          args: [userAddress as `0x${string}`, BigInt(FIRE_DUST_ID)]
        }) as bigint;
  ...
  const handleClaimRewards = async () => {
    ...
    // Verificar que tenemos un publicClient
    if (!publicClient) {
    ...
    try {
      // Intentar primero con el nuevo endpoint migrado a viem
      console.log('Calling viem claim tokens API...');
      const response = await fetch('/api/claim-tokens-viem', {
        method: 'POST',
        headers: {
