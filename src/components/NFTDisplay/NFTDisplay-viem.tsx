...
import { fetchUserNFTs, calculateNFTPoints, PRIMOS_NFT_CONTRACT } from '@/services/nftService-viem';
...
const NFTDisplay: React.FC<NFTDisplayProps> = ({ publicClient: externalPublicClient, userAddress, refreshTrigger, onLoadingStateChange }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setSyncingNFTs(true);
        const result = await fetchUserNFTs(publicClient, userAddress);
    ...
    loadData();
  }, [publicClient, userAddress, refreshTrigger]); // Eliminada la dependencia onLoadingStateChange
